...
--
-- FIX (VISI-11, ao rodar contra `supabase db reset` num banco vazio): REVOKE
-- não aceita IF EXISTS como ALTER TABLE, então num banco sem essa tabela (todo
-- banco criado do zero pelas migrations, já que ela nunca foi versionada) esta
-- linha derrubava o reset inteiro com "relation does not exist". Sem efeito na
-- produção — lá a tabela existe e o REVOKE roda normalmente.
